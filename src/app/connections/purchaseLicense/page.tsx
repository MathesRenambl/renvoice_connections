            ...
            <div className="mt-4">
                ...
                <div className="grid grid-flow-col h-full gap-4">
                                                ...
                                                <td className="py-3 px-4 font-semibold">
                                                    {service.name === 'calls' ? (
                                                        <div className="flex flex-col">
                                                            <span className="pr-3">{`₹${service.amount}`}</span>
                    ...
                    <div className=" h-full">
                        <Card className="sticky top-8 border-2 border-gray-300 shadow-sm">
                            ...
                            <CardContent className="space-y-4 text-sm">
                                ...
                                <InfoRow label="Base Connection Cost" value={`₹${currentPrice.toLocaleString()}`} />
                                {/* <InfoRow label="Services Cost (per connection)" value={`₹${totalServiceCostPerConnection.toLocaleString()}`} /> */}
                                <InfoRow label="Total Connection Price" value={`₹${totalConnectionPrice.toLocaleString()}`} />
                                {serviceTotal > 0 ? (<InfoRow label="Add-on Services" value={`₹${serviceTotal.toLocaleString()}`} />) : ""}
                                {discountPercentage > 0 && (
                                    <div className="flex justify-between text-green-600">
                                        <span className="font-semibold">Connection Discount ({discountPercentage}%)</span>
                                        <span className="font-semibold">- ₹{connectionDiscountAmount.toLocaleString()}</span>
                                    </div>
                                )}
